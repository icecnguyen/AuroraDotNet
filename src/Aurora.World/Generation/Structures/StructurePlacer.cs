...
public static class StructurePlacer
{
    public static void PlaceStructures(Chunk chunk, int chunkX, int chunkZ, int[] highestSolidY, BiomeType biome, int seed)
    {
        ...
        // 2. Desert Well (~1 in 20 chunks in Desert)
        else if (biome == BiomeType.Desert && (hash % 20) == 1)
        {
            TryPlaceDesertWell(chunk, highestSolidY);
        }
        // 3. Explorer Campsite (~1 in 24 chunks in Taiga, Forest, or Plains)
        else if ((biome == BiomeType.Taiga || biome == BiomeType.Forest) && (hash % 24) == 2)
        {
            TryPlaceCampsite(chunk, highestSolidY, hash);
        }
        // 4. Plains Village House (~1 in 28 chunks in Plains or Forest)
        else if ((biome == BiomeType.Plains || biome == BiomeType.Forest) && (hash % 28) == 3)
        {
